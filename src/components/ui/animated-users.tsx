      ...
      <div className="relative grid grid-cols-1 gap-20 md:grid-cols-2">
        <div>
          <div className="relative h-80 w-full">
            <AnimatePresence>
              {contributors.map((testimonial, index) => (
                <motion.div
                  key={testimonial.src}
                  initial={{
                    opacity: 0,
        ...
        <div className="flex flex-col justify-between py-4">
          ...
          >
            ...
            <div className="mt-4 flex items-center gap-x-4">
              {contributors[active]!.links?.map(({ url, icon: Icon }) => (
                <a
                  key={url}
                  href={url}
                  target="_blank"
                ...
                >
                  <Icon size="1.25rem" className="text-muted" />
                </a>
              ))}
            </div>
            ...
            <motion.p className="mt-8 text-lg text-gray-500 dark:text-neutral-300">
              {contributors[active]!.quote.split(" ").map((word, index) => (
                <motion.span
                  key={index}
                  initial={{
                    filter: "blur(10px)",
